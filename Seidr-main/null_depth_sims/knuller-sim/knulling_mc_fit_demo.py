...
dra, ddec, con = 2, 2, 1e-1  # test companion!
wavel = 1.65  # observing wavelength
...
# -----------------------------------------------------
nmc = 2000  # number of MC iterations per piston

p0 = dra * 1.05, ddec * 1.05, con * 1.3  # just to start slightly off the mark!

npst = 5  # number of piston rms values
# prms = np.arange(1, npst + 1) * 10.0  # series of pistons to test for
prms = np.linspace(10, 70, npst)  # series of pistons to test for
...
vplot_widths = np.diff(prms).mean() * 0.5
f1 = plt.figure(figsize=(21, 5))
ax1 = f1.add_subplot(1, 3, 1)
ax1.violinplot(pfit[:, :, 0].T, prms, widths=vplot_widths)
ax1.axhline(dra, color="k", linestyle="--")
ax1.set_xlabel("Residual piston rms (in nanometers)")
ax1.set_ylabel("Best fit location (in mas)")
ax1.set_title("Right ascension offset")

ax2 = f1.add_subplot(1, 3, 2)
ax2.violinplot(pfit[:, :, 1].T, prms, widths=vplot_widths)
ax2.axhline(ddec, color="k", linestyle="--")
ax2.set_xlabel("Residual piston rms (in nanometers)")
ax2.set_ylabel("Best fit location (in mas)")
ax2.set_title("Declination offset")

ax3 = f1.add_subplot(1, 3, 3)
ax3.violinplot(pfit[:, :, 2].T, prms, widths=vplot_widths)
ax3.axhline(con, color="k", linestyle="--")
ax3.set_xlabel("Residual piston rms (in nanometers)")
ax3.set_ylabel("Best fit location (in mas)")
ax3.set_title("Contrast")
